...
app = FastAPI(
    title="CDP活動資料查詢代理 (CDP Campaign Agent)",
    description="一個透過查詢資料庫來回答'特定員工'在建立CDP活動後，活動狀態、活動發送的渠道狀態、發送的內容、發送的結果的代理程式。**注意：呼叫此工具需要有效的身份驗證，客戶端必須在 HTTP Authorization 標頭中提供一個有效的 JWT Bearer Token。**",
    version="0.1.0",
)
...
@app.get("/")
def read_root():
    return {"message": "CDP Campaign Agent is running with custom JWT authentication."}
...
async def execute_query(
    ...
    try:
        ...
        print(f"===================CDP活動 mcp_tools result：{result}")
